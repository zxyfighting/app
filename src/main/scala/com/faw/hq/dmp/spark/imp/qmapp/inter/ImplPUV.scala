...
import com.faw.hq.dmp.spark.imp.qmapp.realize.APKafkaConsumer.UserVisit
import com.faw.hq.dmp.spark.imp.qmapp.util.JedisPoolUtils
...
object ImplPUV {
  ...
  /**
    ...
    * */
  //UV计算-----通过redis进行批次间去重
  def filterRedis(rdd: DStream[UserVisit], ssc:StreamingContext):DStream[UserVisit]  = {
    rdd.transform(rdd=> {
      ...
      val filterUser: RDD[UserVisit] = rdd.filter(rdd => {
  ...
  def groupByUnion(value: DStream[UserVisit]): DStream[UserVisit] = {
 ...
 //UV计算------将结果存到redis中
  def distinctRedis(rdd: DStream[UserVisit]): Unit = {
    rdd.foreachRDD(rdd => {
